...
class KeyboardPolicy:
    ...
    def _print_status(self, newline: bool = False) -> None:
        """Print current status line."""
        prefix = "\r" if not newline else ""
        joint_val = self.action[self.selected_joint]
        msg = f"{prefix}[CTRL] Joint: {self.selected_joint:2d} | Value: {joint_val:+.3f} | "
        msg += f"Steps: {self.step_count:4d}" + " " * 20
    ...
    def update_action(self, key: str) -> bool:
        ...
        if key == "1":
            ...
            self.action[self.selected_joint] = np.clip(self.action[self.selected_joint] - self.action_delta, -1.0, 1.0)
            self._print_status()
...
async def main(host: str = "0.0.0.0", port: int = 8000, action_dim: int = 23, action_delta: float = 0.1):
    ...
    def process_request(connection, request):
        ...
        if request.path == "/healthz":
            ...
            return Response(
                status_code=200, reason_phrase="OK", headers=Headers([("Content-Type", "text/plain")]), body=b"OK\n"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Keyboard-controlled WebSocket server for BEHAVIOR environment")
    parser.add_argument("--host", type=str, default="0.0.0.0", help="Host address to bind to (default: 0.0.0.0)")
    parser.add_argument("--port", type=int, default=8000, help="Port to listen on (default: 8000)")
    parser.add_argument(
        "--action-dim", type=int, default=23, help="Action space dimension for R1Pro robot (default: 23)"
    )
    parser.add_argument(
        "--delta", type=float, default=0.1, help="Amount to change joint value per keypress (default: 0.1)"
    )
    ...
    # Run server
    try:
        asyncio.run(main(host=args.host, port=args.port, action_dim=args.action_dim, action_delta=args.delta))
